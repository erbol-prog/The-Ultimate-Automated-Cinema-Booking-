...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.home_url = QUrl("http://localhost:8000")
    ...
    def setCurrentIndexAnimated(self, index):
        ...
        current_animation.setEndValue(
            current_widget.pos()
            + (-self.width() if index > self.currentIndex() else self.width(), 0)
        )
    ...
    def menu_expand(self):
        ...
        intermediate_geometry = QRect(
            self.menu.geometry().x(), self.menu.geometry().y(), 60, 220
        )
        ...
        full_geometry = QRect(
            self.menu.geometry().x(), self.menu.geometry().y(), 170, 220
        )
        ...
        if self.menu_show_up:
            ...
            for label in [
                self.menu_lays.label,
                self.home_lays.label,
                self.back_lays.label,
                self.quit_lays.label,
            ]:
            ...
            for button in [
                self.home_lays.button,
                self.back_lays.button,
                self.quit_lays.button,
            ]:
